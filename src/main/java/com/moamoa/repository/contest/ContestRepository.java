...
public interface ContestRepository extends JpaRepository<Contest, Long>, ContestRepositoryCustom {
  ...
  @org.springframework.data.jpa.repository.Query(
      "select c from Contest c where c.endDate=:date and c.sourceClosed=false")
  List<Contest> findByEndDate(LocalDate date);

  @org.springframework.data.jpa.repository.Query(
      "select c from Contest c where c.id<>:id and c.category=:category and c.endDate>=:today and"
          + " c.sourceClosed=false order by case when c.district=:district then 0 else 1"
          + " end,c.endDate,c.id")
